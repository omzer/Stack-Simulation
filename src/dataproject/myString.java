...
public myString(String s){input =s;}
...
while(tk.hasMoreTokens()){
    String a = tk.nextToken();
      ...
      post+=operators.pop()+" ";
        if(operators.peek()==null){post=correct(post);break;}
      }
      operators.pop();
      continue;
      }
      ...
      if(a.equals("("))
      {operators.push(a);continue;}
   
      if(operators.getSize()==0||getPresedence(a)>getPresedence(operators.peek()+"")||(operators.peek()+"").equals("("))
        operators.push(a);
      else{ 
...
while(operators.getSize()!=0){post+=operators.pop()+" ";}
return post;
}
